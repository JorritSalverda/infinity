	...
	t.Run("RunsOnce", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			...
			go func(i int) {
				...
				log.Printf("Running semaphore test goroutine %v...", i)
	...
	t.Run("ResetsAfterWaitToBeUsedAnotherTime", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			...
			go func(i int) {
				...
				log.Printf("Running first semaphore test goroutine %v...", i)
				time.Sleep(100 * time.Millisecond)
			}(i)
		}
		...
		for i := 0; i < 5; i++ {
			...
			go func(i int) {
				...
				log.Printf("Running second semaphore test goroutine %v...", i)
				time.Sleep(100 * time.Millisecond)
			}(i)
		}
